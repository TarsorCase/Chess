...
class Node:
    ...
    def setup(self, WIN, boardM):
        if boardM[self.row][self.col] != "None":
            piece_type = boardM[self.row][self.col].lower()  # Get lowercase piece type
            image_path = f"imagenes/{piece_type}.png"  # Combine with ".png"
            WIN.blit(pygame.image.load(image_path), (self.x, self.y))

    def getImage(self, letter):
        # Assuming image paths are stored in a dictionary 'piece_images'
        return piece_images[letter]

    # Pre-load piece images into a dictionary for faster access
    piece_images = {}
    for piece_type in ['r', 'n', 'b', 'q', 'k', 'p', 'R', 'N', 'B', 'Q', 'K', 'P']:
        image_path = f"D:\\UCB\\Intro a programaccion\\Ajederez\\imagenes\\{piece_type}.png"
        piece_images[piece_type] = pygame.image.load(image_path)
